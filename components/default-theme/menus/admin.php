...
<menu type="context" id="admin_menu">
	...
	<menu label="PHP Defaults">
			<menuitem type="command" label="_SERVER" icon="images/octicons/svg/tools.svg" data-request="debug=_SERVER&nonce=<?=$session->nonce?>"></menuitem>
			<menuitem type="command" label="_SESSION" icon="images/octicons/svg/tools.svg" data-request="debug=_SESSION&nonce=<?=$session->nonce?>"></menuitem>
			<menuitem type="command" label="_COOKIE" icon="images/octicons/svg/tools.svg" data-request="debug=_COOKIE&nonce=<?=$session->nonce?>"></menuitem>
			<menuitem type="command" label="Headers" icon="images/octicons/svg/tools.svg" data-request="debug=headers&nonce=<?=$session->nonce?>"></menuitem>
			<menuitem type="command" label="PHP Extensions" icon="images/octicons/svg/tools.svg" data-request="debug=extensions&nonce=<?=$session->nonce?>"></menuitem>
			<menuitem type="command" label="Apache Modules" icon="images/octicons/svg/tools.svg" data-request="debug=modules&nonce=<?=$session->nonce?>"></menuitem>
		</menu>
	<menu label="Manage Database">
		<menuitem type="command" label="Restore Database" icon="images/octicons/svg/database.svg" data-request="action=restore database&nonce=<?=$session->nonce?>" data-confirm="Are you sure you want to restore the database from <?=$connect->database?>.sql? All changes made since the last mysqldump will be reverted."></menuitem>
		<menuitem type="command" label="Backup Database" icon="images/octicons/svg/database.svg" data-request="action=backup database&nonce=<?=$session->nonce?>" data-confirm="Are you sure you want to backup the database to <?=$connect->database?>.sql?"></menuitem>
		<menuitem type="command" label="Clear PHP Errors" icon="images/octicons/svg/bug.svg" data-request="action=Clear PHP_errors&nonce=<?=$session->nonce?>"></menuitem>
		<menuitem type="command" label="Clear CSP" icon="images/octicons/svg/bug.svg" data-request="reset_table=CSP_errors&nonce=<?=$session->nonce?>"></menuitem>
		</menu>
