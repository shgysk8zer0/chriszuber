<?php
	...
	set_include_path(__DIR__ . DIRECTORY_SEPARATOR . 'classes' . PATH_SEPARATOR . get_include_path() . PATH_SEPARATOR  . __DIR__  . DIRECTORY_SEPARATOR . 'config');
	...
	try {
		...
		if(
			...
			$DB =new \core\PDO('connect');

			if(!$DB->connected) {
				throw new \Exception('Database connection failed', 504);
			}

			$DB->prepare("INSERT INTO `CSP_errors` (
					`blocked-uri`,
					`document-uri`,
					`violated-directive`,
					`source-file`,
